...
- **Operators (this repo):** `gpt-6-astra` (reasoning `high`) sees only the schema and ≤ 20 sample rows and writes a typed plan; `jev-1.13.0` answers the per-row semantic questions; DuckDB does the filtering, sorting, joins and arithmetic.
...
Prices used (list, 2026-09-21): gpt-6-astra $10.00 / M input, $1.00 / M cached input, $50.00 / M output (reasoning tokens bill as output; requests over 272,000 input tokens reprice to $20.00 / $75.00); jev-1.13.0 $0.042 / M input, output free. Costs below are computed from the token usage each API reported.
...
| **Consumer complaints: filter then rank by urgency** | 5,000 | 0 rows ranked; 10 rows in review view (8 of the one-shot's picks among them) | 8 rows ranked; 100.0% carry a topical keyword | Jaccard 0.00; Spearman ρ – on 0 shared; top-20 overlap 0 |
